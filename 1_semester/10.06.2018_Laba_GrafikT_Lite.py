...
if 0<=y0<=size:
    ...
    ystep=size//3
    print(ystep)
    print('  {}{:4.2e}{}{:4.2e}'.format(' '*(ystep - 1),ystep*(maxz - minz)/size+minz,
                                        ' '*(ystep-8),2*ystep*(maxz-minz)/size+minz))
    print('  {:4.2e}{}{:4.2e}'.format(minz,' '*(size-8),maxz))
    print('    ┌┼','─'*(ystep - 1),'┼','─'*(ystep - 1),'┼','─'*(size - ystep * 2),
          '┼','─'*8,'► Y',sep='')
    x=d1*10
    n=0
    while n<=l:
